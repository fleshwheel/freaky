...
@click.option("-w", "--window-length", default=32, help="Width of analysis windows.")
...
def decode(spectra, sample_rate, window_length):
    ...
    freqs = (FREQ_MAX * np.linspace(0, 1, spectra.shape[1])).astype(np.uint)
    print(len(freqs))
...
#    spectra = np.exp(spectra)
    ...
    phases = np.random.random(len(freqs)) * sample_rate * 4 * 16
